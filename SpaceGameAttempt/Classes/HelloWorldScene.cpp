...
void HelloWorld::update(float dt){
    ...
    //move the asteroids
    float curTimeMills = getTimeTick();
    if(curTimeMills > _nextAsteroidSpawn){
        float randomMills = randomValueBetween(0.2, 1.0) * 1000;
        ...
        asteroid->setVisible(true);
        asteroid->runAction(
                Sequence::create(MoveBy::create(randDuration, Vec2(-winSize.width-asteroid->getContentSize().width/2, 0)),
...
float HelloWorld::randomValueBetween(float min, float max){ return (((float)arc4random()/0xFFFFFFFFu) * (max - min)) + min; }
float HelloWorld::getTimeTick(){
    timeval time;
    gettimeofday(&time, NULL);
    unsigned long millisecs = (time.tv_sec * 100) + (time.tv_usec/1000);
